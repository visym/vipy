...
def tile(T, nrows=16, ncols=16):
    ...
    Returns:
        vipy.image.Image with float32 numpy array of size (T.shape[0]*nrows, T.shape[1]*ncols)
    """
    img = None
    for i in range(0,nrows):
...
def dots(dx=16, dy=16, nrows=8, ncols=8):
    ...
    Returns:
        vipy.image.Image with float32 numpy array in range [0,1]
    """
...
def vertical_gradient(nrows=512, ncols=512):
    ...
    """
    return vipy.image.Image(array=np.outer([(255.0 * (x / float(nrows))) for x in range(0,nrows)], np.ones((1,ncols))).astype(np.uint8), colorspace='lum')
...
def centersquare_border(height=512, width=512, squaresize=None, channels=1):
    ...
    img[x-s:x+s+1, y+s] = 1.0    
    return vipy.image.Image(array=np.uint8(img*255), colorspace='lum')


def circle(x=256, y=256, r=128, width=512, height=512, channels=1):
    ...
    return vipy.image.Image(array=img, colorspace='float')


def stepup(width=512, height=512):
...
def stepdown(width=512, height=512):
    """Create a left black/right white step image of size (width, height)
    
    Returns:
        `vipy.image.Image` object
    """
    return stepup(width, height).fliplr()
