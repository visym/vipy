...
def scene_search(im, outfile=None):
    ...
    <title>scene_search</title>
    <style>
      #imageContainer {
        position: relative;
        width: ${IMG_WIDTH}px; /* Set width of image container */
        overflow: hidden; /* Hide overflow for zoomed-in image, set this to auto to allow for scrollbars, but need to disable for scale<1, and add panning again
      } 
      #mainImage {
        position: absolute;
    top: 0;
    left: 0;
    width: 100%; /* Ensure image fits container initially */
    ...
    touch-action: none; /* Disable browser default touch actions */
      ...
      #searchBar {
        ...
        z-index: 10; /* Ensure the search bar is above the image */
      }
  #searchInput {
  ...
  #searchDropdown {
    ...
    z-index: 10; /* Ensure the dropdown is above the image */
    opacity: 0.8;
  }
  #searchDropdown li {
    padding: 0px;
  ...
  .highlighted {
    background-color: #ddd;
      ...
      .caption {
        position: absolute;
        z-index: 20; /* Ensure the captions are above the overlay */
        color: white;
        font-size: 32px;
      }
      #menuButton {
         position: absolute;
         top: 10px; /* Adjust top position of hamburger menu */
         right: 10px; /* Adjust right position of hamburger menu */
         z-index: 10; /* Ensure the hamburger menu is above the image */
      }
    ul {
    ...
    //height: 200px; 
    padding: 10px;
    z-index: 100; /* Ensure the popup is above the image */
  ...
  .line {
    ...
    z-index: 99; /* Ensure the line is behind the popup */
    ...
    <div id="imageContainer">
      <img id="mainImage" src="${IMG}" alt="Main Image" width="${IMG_WIDTH}" > 
      <div id="popup" class="annotationPopup">This is the annotation text</div>
      <div id="line" class="line"></div>
      <div id="searchBar">
        <input type="text" id="searchInput" placeholder="Search...">
        <ul id="searchDropdown"></ul>    
      </div>
      <!-- <div class="caption" style="top: 250px; left: 250px;">Caption 1</div>
      <div class="caption" style="top: 400px; left: 400px;">Caption 2</div> -->
      <!-- <div id="menuButton"><button onclick="toggleMenu()">☰</button> </div> -->
    ...
    function overlaySearchBar() {
      ...
      // Populate dropdown with options (replace with your own data)
      ...
      // Show search bar on image hover
      //imageContainer.addEventListener('mouseover', function() {
      //  searchBar.style.display = 'block';
      //});
    
      // Hide search bar on mouseout
  ...
  // Function to show popup at specific coordinates
  function showPopup(x, y) {
    popup.style.display = 'block';
    popup.style.left = (x + 5) + 'px'; // Offset by 5px to the right
    popup.style.top = (y + 5) + 'px'; // Offset by 5px downwards

    // Draw line connecting clicked location and popup
    var centerX = x + mainImage.offsetLeft + mainImage.offsetWidth / 2;
    var centerY = y + mainImage.offsetTop + mainImage.offsetHeight / 2;
    var angle = Math.atan2(centerY - parseInt(popup.style.top, 10), centerX - parseInt(popup.style.left, 10));
    var length = Math.sqrt(Math.pow(centerY - parseInt(popup.style.top, 10), 2) + Math.pow(centerX - parseInt(popup.style.left, 10), 2));
    line.style.width = length + 'px';
    line.style.transform = 'rotate(' + angle + 'rad)';
    line.style.left = (parseInt(popup.style.left, 10) + popup.offsetWidth / 2) + 'px';
    line.style.top = (parseInt(popup.style.top, 10) + popup.offsetHeight / 2) + 'px';
    line.style.display = 'none';  // disable me
  }
  ...
  // Event listener to detect clicks on the image
  imageContainer.addEventListener('click', function(event) {
    var offsetX = event.x; // X-coordinate relative to the image container
    var offsetY = event.y; // Y-coordinate relative to the image container

    // Show popup at the clicked coordinates
    var isClickInsidePopup = popup.contains(event.target);
    if (isClickInsidePopup) {
      hidePopup();
    }
    else {
      showPopup(offsetX, offsetY);
    }
  });


  var scale = 1; // Initial scale
  var lastX, lastY; // Last touch position
  ...
  function handleTouchMove(event) {
    ...
    lastX = touch.clientX;
    lastY = touch.clientY;
    mainImage.style.left = (mainImage.offsetLeft + deltaX) + 'px';
    mainImage.style.top = (mainImage.offsetTop + deltaY) + 'px';
  }
  ...
  function handlePinch(event) {
    ...
    if (touch1 && touch2) {
      ...
      var delta = distance - event.distance;
      var newScale = scale + delta * 0.01; // Adjust zoom sensitivity
      if (newScale >= 0.2) { // Ensure minimum scale
        scale = newScale;
  ...
  // Event listener to detect pinch gestures
  imageContainer.addEventListener('touchstart', function(event) {
    if (event.touches.length === 2) {
      imageContainer.addEventListener('touchmove', handlePinch);
      imageContainer.addEventListener('touchend', function() {
        imageContainer.removeEventListener('touchmove', handlePinch);
      });
    }
  });
    }
    ...
    function search() {
      var searchTerm = document.querySelector('#searchBar input').value;
      // Perform search operation here, e.g., display search results
    ...
    html = template    
    keywords = {'${IMG_WIDTH}':768,                      
                '${IMG}':im.url() if im.has_url() else 'data:image/jpeg;charset=utf-8;base64,%s' % im.resize(width=768).base64().decode('ascii'),
                '${SEARCHBOX_WIDTH}':int(768*0.25),
