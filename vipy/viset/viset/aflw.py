...
VISET = 'aflw'
VISETDIR = os.path.join(Cache().root(), VISET)
...
def rdd(sparkContext=None, appName='aflw', verbosity=1):
    sparkContext = bobo.app.init(appName=appName, verbosity=verbosity) if sparkContext is None else sparkContext        
    csvfile = os.path.join(VISETDIR, '%s.csv' % VISET)
    cache = Cache()
    
    # Parse CSV file based on protocol, all non-detection properties go in attributes dictionary
    return (sparkContext.textFile(csvfile, minPartitions=bobo.app.num_cores())
            .map(lambda row: row.encode('utf-8').split(','))  # CSV to list of row elements
            .filter(lambda x: x[0][0] != '#')  # hack to ignore comments
            .map(lambda x: ImageDetection(url=cache.key(os.path.join(VISETDIR, x[0])), category=None,
...
def export():
    dbfile = os.path.join(VISETDIR, 'data', 'aflw.sqlite')
    db = sqlite3.connect(dbfile)
    cursor = db.cursor()
    
    outfile = os.path.join(VISETDIR, '%s.csv' % VISET)
